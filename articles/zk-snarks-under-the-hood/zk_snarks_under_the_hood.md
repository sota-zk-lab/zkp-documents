...
- B is the linear combination of ${B_1…B_m}$ with the same coefficients as A
- C is a linear combination of ${C_1…C_m}$ with the same coefficients as A

> [!NOTE]
> Note that from this step, we are going to use [elliptic curve](../../terms/elliptic_curve.md), so we will have a generator point $G$ (corresponding to 1 in the elliptic curve) appear in our equation as a way to convert a normal number into an elliptic curve.
...
Here, we use random secret variables $t, k_a, k_b, k_c$ generated by the verifier, then encrypt them and send those encrypted numbers to the prover. $t$ will be a point where we evaluate our polynomial. 
